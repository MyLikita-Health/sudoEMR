...
import { DoctorGuide } from '../utils/Guides';
...
class PatienClarking extends React.Component {
  ...
  fetchDiagnosis(patientId) {
    _fetchApi(
      `${apiURL()}/diagnosis/fullDiagnosis?q=${patientId}`,
      (data) => this.setState({ fullDiagnosis: data }),
      (err) => console.log(err),
    );
  }
  ...
  fetchData() {
    _fetchApi(
      `${apiURL()}/patientrecords/doctor?q=${'aminu'}`,
      (data) => this.setState({ patientrecords: data }),
