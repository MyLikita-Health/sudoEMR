...
import { _warningNotify, checkEmpty } from "../utils/helpers";
...
import LabRequestForm from "./lab-test/LabRequestForm";

class NewRadiologyInvestigations extends Component {
  state = {
    investigation: {
      test: "",
      sample: "",
  ...
  handleAdd = (selected, receipt, amount) =>
    this.setState((prev) => ({
      ...prev,
    ...
    return (
      <Card>
        ...
        <CardBody style={{ height: 400 }}>
          {/* {JSON.stringify({ labTest, investigation: this.state.investigation })} */}
          <Scrollbars>
            <LabRequestForm
              note={note}
              ...
              amount={amount}
              handleAdd={handleAdd}
              handleNoteChange={this.onInputChange}
            />
          </Scrollbars>
        </CardBody>
